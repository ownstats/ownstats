...
export const deriveEventDetails = (s3Event) => {
  // See https://docs.aws.amazon.com/AmazonS3/latest/userguide/notification-content-structure.html
  const eventName = s3Event.Records[0].eventName.split(':')[0];
  const s3Key = decodeURIComponent(s3Event.Records[0].s3.object.key);
  const s3Bucket = s3Event.Records[0].s3.bucket.name;
  const fileCreatedTimestamp = s3Event.Records[0].eventTime;
  const fileSize = s3Event.Records[0].s3.object.size;
