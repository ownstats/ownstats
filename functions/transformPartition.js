    ...
    if (partitionExistsResult.hasOwnProperty('Contents') && partitionExistsResult.Contents.length > 0) {
      ...
      const ctasStatement = `
        ...
        AS SELECT 
          ...
          substr(location, 1, 3) as edge_location_prefix,
          bytes,
          ...
          cast(url_decode(url_extract_parameter(qs, 'd')) as int) as device_color_depth,
          url_decode(url_extract_parameter(qs, 'p')) as device_platform,
          cast(url_decode(url_extract_parameter(qs, 'm')) as int) as device_memory,
          cast(url_decode(url_extract_parameter(qs, 'c')) as int) as device_cores,
